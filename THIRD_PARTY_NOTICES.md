...
| [`@img/sharp-wasm32`](https://github.com/lovell/sharp) | Apache-2.0 AND LGPL-3.0-or-later AND MIT |
...
`@img/sharp-wasm32` (Apache-2.0 AND LGPL-3.0-or-later AND MIT), `eslint-plugin-sonarjs` (LGPL-3.0-only) and `lightningcss` (MPL-2.0) run only as development tooling; their code is not linked into or distributed with any DeepSeek Harness artifact.
