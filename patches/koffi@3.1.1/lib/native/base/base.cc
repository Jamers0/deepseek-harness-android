                        ...
                        FunctionRef<void(Span<uint8_t> buf)> out_func, int *out_code)
{
    ...
    pid_t pid;
    {
        ...
        pid = fork();
        if (pid < 0) {
            LogError("Failed to fork process: %1", strerror(errno));
            return false;
        }
        if (pid == 0) {
            if (in_func.IsValid()) dup2(in_pfd[0], STDIN_FILENO);
            if (out_func.IsValid()) {
...
#if defined(__ANDROID__)
        if (info.work_dir) chdir(info.work_dir);

        if (in_func.IsValid()) close(in_pfd[1]);
        if (out_func.IsValid()) close(out_pfd[0]);
